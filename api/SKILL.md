...
4. **IDs are prefixed strings.** `vg_work_...`, `vg_tool_...`, `vg_file_...`, `vg_rmix_...`, `vg_voic_...`, `vg_enti_...`. Store as-is. On `voiceId` fields you may also pass a catalog `displayName` (e.g. `Matilda`).
...
Reusable actors, products, visual styles, and slideshow themes shared across your team. Attach their reference images to workflows for a consistent character (`ACTOR`) or look (`VISUAL_STYLE`). `PRODUCT` entities are a reusable library of product reference images. `SLIDESHOW_THEME` entities define a shared slide design system (fonts, colors, layout) applied to every slide of a slideshow deck. Use an entity in a workflow by passing its `vg_enti_...` id: `visualStyle: { type: "AI_IMAGE", entityId }` (script + voiceover), per-scene `actorEntityIds` / `productEntityIds` / `visualStyleEntityId` on `storyboardToVideo`, or `slideshowThemeEntityId` on `slideshowToVideo`. For avatars, pass an ACTOR entity as `actorEntityId` and optionally set `avatarQuality` (`LOW`, `STANDARD`, `HIGH`, or `MAX`) on `generateAvatar`, script-to-video, slideshow-to-video, or `CHANGE_NARRATOR`. Direct avatar generation also requires `audioFileId` and exactly one avatar source.

| Method                  | Endpoint                                       | Description                                                          |
| ----------------------- | ---------------------------------------------- | ------------------------------------------------------------------- |
| `listEntities`          | `GET /v1/entities`                             | List entities (filter with `?entityType=ACTOR`, `PRODUCT`, `VISUAL_STYLE`, or `SLIDESHOW_THEME`) |
